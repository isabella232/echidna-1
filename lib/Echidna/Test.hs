...
testTx :: (Has TestConf x, Has TxConf x, MonadReader x m)
       => (FunctionName, Addr) -> m Tx
testTx (f, a) = do
  TestConf _ s <- view hasLens
  g            <- view (hasLens . propGas)
  pure $ Tx (SolCall (f, [])) (s a) a g 0 0 (0, 0)
...
checkETest t = do
  ...
  res <- case t of
    ...
    Left  test -> execTx (testTx test conf) >> gets (p (fst test) . getter)
