...
instance FromJSON SlitherInfo where
  parseJSON = withObject "slitherOutput" $ \o -> do
    ...
    description <- ((o .: "results" >>= (.: "printers")) <&> head) >>= (.: "description")
    ...
    where
      parseDescription = withObject "description" $ \o -> do
        ...
        generationGraph <-
          o .: "functions_relations" >>= (traverse . traverse) (withObject "relations" (.: "impacts"))
        pure SlitherInfo {..}
        ...
        case t of
          ...
          "address" ->
            case readMaybe v :: Maybe Int of
              Just a ->
                case readMaybe ("0x" ++ showHex a "") of
                  Nothing -> failure v t
                  a' -> pure $ AbiAddress <$> a'
              Nothing -> failure v t
          ...
          _ -> pure Nothing
        where failure v t = fail $ "failed to parse constant: "++ t ++ " " ++ v
